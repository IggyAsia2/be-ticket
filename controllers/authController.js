...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError("Mật khẩu vừa thay đổi, xin đăng nhập lại.", 401));
  }
...
exports.permission = (value) => {
  return async (req, res, next) => {
    const doc = await Role.findById(req.user.role).populate({ path: "rights" });
    for (let i of doc.rights) {
      if (i.name === value) {
        return next();
      }
    }
...
exports.forgotPassword = async (req, res, next) => {
  ...
  // 2) Genearate the random reset token
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (await user.correctPassword(req.body.password, user.password)) {
    return next(new AppError("Mật khẩu mới của bạn giống mật khẩu cũ.", 401));
  }
